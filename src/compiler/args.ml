...
let parse_args com =
	...
	let adv_args_spec = [
		...
		(* Preprocessor scripts: run external commands that transform file text before parsing *)
		("Compilation",["--preprocessor"],["-pp"], Arg.String (fun cmd ->
			(* store raw command line; it will be executed with the file contents on stdin *)
			com.preprocessors <- com.preprocessors @ [cmd]
		),"<command>","register a textual preprocessor command to run before parsing (can be given multiple times)");
