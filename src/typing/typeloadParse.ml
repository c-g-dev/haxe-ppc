...
(* toplevel prototype removed; preprocessors are now configured via CLI and run as external commands *)
open Globals
open Ast
open Parser
...
let run_preprocessor_command (cmd : string) (input : string) : string =
	let (stdout_ch, stdin_ch, stderr_ch) = Unix.open_process_full cmd (Unix.environment ()) in
	output_string stdin_ch input;
	flush stdin_ch;
	close_out stdin_ch;
	let output = Std.input_all stdout_ch in
	match Unix.close_process_full (stdout_ch, stdin_ch, stderr_ch) with
		| Unix.WEXITED 0 -> output
		| Unix.WEXITED code -> failwith (Printf.sprintf "Preprocessor '%s' exited with code %d" cmd code)
		| Unix.WSIGNALED s -> failwith (Printf.sprintf "Preprocessor '%s' was signaled (%d)" cmd s)
		| Unix.WSTOPPED s -> failwith (Printf.sprintf "Preprocessor '%s' was stopped (%d)" cmd s)

let apply_preprocessors (com : Common.context) (text : string) : string =
	List.fold_left (fun acc cmd -> run_preprocessor_command cmd acc) text com.preprocessors
...
let parse_file com rfile p =
	...
	let contents = match com.file_contents with
			| [] when (Common.defined com Define.DisplayStdin) && DisplayPosition.display_position#is_in_file file_key ->
					...
					close_in stdin;
					let s = if com.preprocessors = [] then s else apply_preprocessors com s in
	...
	| _ ->
			...
			| FFile ->
					...
					if com.preprocessors <> [] then begin
						let data = Std.input_file ~bin:true file in
						let s = apply_preprocessors com data in
